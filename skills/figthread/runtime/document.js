...
function validateAuthorities(a, canonical){
  ...
  if(l.grammar_plan_hash!==g.plan_hash)issues.push(issue("DOC004_COMPILE","error","layout is not bound to the promoted grammar plan"));
  ...
  if(l.profile_plan_hash!==p.plan_hash||r.profile_plan_hash!==p.plan_hash)issues.push(issue("DOC004_COMPILE","error","profile-plan identity diverges across layout/render authorities"));
