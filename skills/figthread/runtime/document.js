...
const summaryState=()=>{const fig=manifest?.canonical?.figure;if(!fig)return {};const snap=(fig.snapshots||[]).find(x=>x.id===fig.static_snapshot_id);if(snap)return clone(snap.state_values||{});const out={};for(const state of fig.states||[])out[state.id]=state.summary;return out};
...
const sampleState=ms=>{const p=manifest.compiled.motion_program;if(!p)return summaryState();const t=localTime(ms),state=clone(p.initial_state||{});for(const track of p.tracks||[]){if(track.at_ms>t)continue;for(const effect of track.effects||[]){const cur=state[effect.state_id];state[effect.state_id]=effect.op==="add"?cur+effect.value:effect.value}}return state};
const phase=(cue,t)=>{if(t<cue.start_ms)return -1;const end=cue.start_ms+Math.max(0,cue.duration_ms);if(t>end)return 2;if(cue.duration_ms<=0)return 1;return Math.max(0,Math.min(1,(t-cue.start_ms)/cue.duration_ms))};
...
function tick(now){if(!playing)return;const p=manifest.compiled.motion_program,next=playBase+(now-playStart);if(p?.loop?.mode!=="repeat"&&next>=p.duration_ms){renderAt(p.duration_ms);pause();return}renderAt(next);raf=requestAnimationFrame(tick)}
function play(){if(!manifest.runtime.has_motion||mode!=="interactive")return false;if(playing){pause();return false}const p=manifest.compiled.motion_program;if(p?.loop?.mode!=="repeat"&&currentMs>=p.duration_ms)currentMs=0;playing=true;playStart=performance.now();playBase=currentMs;const b=document.querySelector("#figthread-play");if(b)b.textContent="Pause";raf=requestAnimationFrame(tick);return true}
...
window.Figthread=Object.freeze({
  ...
  renderAt,
  setMode,
  prepareExport:async()=>{setMode("static");return Object.freeze({target_id:manifest.runtime.target_id,build_hash:manifest.build_hash,svg_hash:manifest.compiled.rendered.svg_hash,state_hash:await sha(manifest.compiled.motion_program?.static_state||summaryState())})},
  getStateHash:async()=>sha(mode==="static"?(manifest?.compiled?.motion_program?.static_state||summaryState()):sampleState(currentMs)),
