...
function usage() {
  console.error("usage: render.mjs <figure-spec.json> <visual-spec.json> <layout-target.json> [--mode draft|gate] [--promote] [--out figure.svg] [--evidence evidence.json]");
}
...
const valueIndexes = new Set([modeIndex + 1, outIndex + 1, evidenceIndex + 1].filter((index) => index > 0));
const flags = new Set(["--promote", "--mode", "--out", "--evidence"]);
const positional = args.filter((arg, index) => !flags.has(arg) && !valueIndexes.has(index));
if (positional.length !== 3 || !["draft", "gate"].includes(mode) || (outIndex >= 0 && !args[outIndex + 1]) || (evidenceIndex >= 0 && !args[evidenceIndex + 1])) {
  usage();
  process.exitCode = 2;
} else {
  try {
    ...
    if (!figurePromotion.promoted) {
      console.log(JSON.stringify({ stage: "semantic", promoted: false, report: figurePromotion.report }, null, 2));
      process.exitCode = 1;
    } else {
      const visualPromotion = promoteVisualSpec(figurePromotion, visual);
      if (!visualPromotion.promoted) {
        console.log(JSON.stringify({ stage: "visual", promoted: false, report: visualPromotion.report }, null, 2));
        process.exitCode = 1;
      } else {
        const profilePromotion = promoteProfilePlan(figurePromotion, visualPromotion, target);
        if (!profilePromotion.promoted) {
          console.log(JSON.stringify({ stage: "profile", promoted: false, report: profilePromotion.report }, null, 2));
          process.exitCode = 1;
        } else {
          const layoutPromotion = promoteProfileLayout(figurePromotion, visualPromotion, profilePromotion);
          if (!layoutPromotion.promoted) {
            console.log(JSON.stringify({ stage: "layout", promoted: false, report: layoutPromotion.report }, null, 2));
            process.exitCode = 1;
          } else {
            ...
            const ok = promote ? result.promoted : report.status !== "fail";
    ...
    console.error(JSON.stringify({ stage: "render", status: "fail", error: error.message }, null, 2));
    process.exitCode = 2;
  }
}
