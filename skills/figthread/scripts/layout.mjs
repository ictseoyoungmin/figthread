...
import { compilePromotedLayout, promoteResolvedLayout } from "../runtime/layout.js";
...
if(positional.length!==2||!["draft","gate"].includes(mode)){usage();process.exitCode=2;} else {
  try {
    ...
    if(!figurePromotion.promoted){console.log(JSON.stringify({stage:"figure-ir",promoted:false,report:figurePromotion.report},null,2));process.exitCode=1;}
    else { const result=promote?promoteResolvedLayout(figurePromotion,request):compilePromotedLayout(figurePromotion,request,{mode}); console.log(JSON.stringify(result,null,2)); const ok=promote?result.promoted:result.status!=="fail"; if(!ok)process.exitCode=1; }
