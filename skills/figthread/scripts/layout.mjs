...
import { compileVisualLayout, promoteVisualLayout } from "../runtime/visual-layout.js";
...
if(positional.length!==3||!["draft","gate"].includes(mode)){usage();process.exitCode=2;}else{
  try{
    ...
    if(!figurePromotion.promoted){console.log(JSON.stringify({stage:"semantic",promoted:false,report:figurePromotion.report},null,2));process.exitCode=1;}
      ...
      else{const result=promote?promoteVisualLayout(figurePromotion,visualPromotion,target):compileVisualLayout(figurePromotion,visualPromotion,target,{mode});console.log(JSON.stringify(result,null,2));const ok=promote?result.promoted:result.status!=="fail";if(!ok)process.exitCode=1;}
    }
