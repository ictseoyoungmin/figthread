...
test("packages independently promoted target documents without CSS geometry scaling",async()=>{
  ...
  const scripts=[...result.figthread_document_package.html.matchAll(/<script(?:\s[^>]*)?>([\s\S]*?)<\/script>/g)].map(x=>x[1]);assert.ok(scripts.length>=2);assert.doesNotThrow(()=>new Function(scripts.at(-1)));
});

test("package identity and bytes are independent of caller target order",async()=>{const children=await promotedChildren(),options={packageId:"package:deterministic",defaultTargetId:"web-wide"};const a=promoteFigthreadPackage(children,options),b=promoteFigthreadPackage([...children].reverse(),options);assert.equal(a.promoted,true);assert.equal(b.promoted,true);assert.equal(a.figthread_document_package.manifest.package_hash,b.figthread_document_package.manifest.package_hash);assert.equal(a.figthread_document_package.html_hash,b.figthread_document_package.html_hash);assert.equal(a.promotion_receipt.promotion_hash,b.promotion_receipt.promotion_hash);});

test("targets may choose different profiles while preserving the same semantic figure",async()=>{const {figure,visual,request,motion}=await fixtures();const web=promoteChild(figure,visual,request.targets[0].layout_target,motion);const presentation={schema_version:"figthread.layout-target/0.1",target:{id:"slides",profile:"presentation",viewport:{width:1280,height:720},safe_area:{top:80,right:80,bottom:80,left:80}},options:{preferred_gap:48,min_gap:24,container_padding:24,routing_grid:4,obstacle_margin:10,max_crossings:4}};const slides=promoteChild(figure,visual,presentation,null);const result=promoteFigthreadPackage([web,slides],{packageId:"package:cross-profile",defaultTargetId:"slides"});assert.equal(result.promoted,true);assert.deepEqual(result.figthread_document_package.manifest.target_order,["slides","web-wide"]);assert.deepEqual(result.figthread_document_package.manifest.targets.map(x=>x.profile),["presentation","technical-explainer"]);});
